...
public abstract class PropertiesHelper {
	...
	public static String findProperty(final Properties properties, final String property) {
		...
		if (value == null)
			throw new IllegalArgumentException(String.format("Property `%s' was not found.", property));
		if (!(value instanceof String))
			throw new IllegalArgumentException(String.format("Property `%s' is not a string.", property));
		result = (String) value;
		if (StringHelper.isBlank(result))
			throw new IllegalArgumentException(String.format("Property `%s' is blank.", property));
